...
const Invoice = ({ data, logoUrl, taxes = [], discount = 0 }) => {
    ...
    const fetchCompanyDetails = async () => {
      try {
        ...
        if (response && response.data) {
          ...
          setSellerState(response?.data?.user?.information?.state?.trim().toLowerCase() || "")
        } else {
  ...
  console.log("add the adjustment amount", Number(Number(adjustAmount)+Number(advanceAmount)));
  ...
  return (
    <div className="p-6 border border-gray-800 max-w-2xl mx-auto font-sans text-sm">
      {/* Branding Section */}
          ...
          {isSameState && (
            <div className="flex  flex-col">
              <div>
                ...
                <span className=" ml-2 text-sm">{Number(totalCGST.toFixed(2))}</span>
              </div>
              <div>
          ...
          {!isSameState && (
            <div className="flex flex-col">
              <div>
                ...
                <span className="ml-2 text-sm">  {Number(totalIGST).toFixed(2)}</span>
              </div>
            </div>
          )}
          ...
          <div className="flex flex-col mt-2">
            {
              Number(adjustAmount)> 0 && (
                <div>
                <span className="font-semibold text-sm">Adjustment Amount:</span>
                <span className="ml-2 text-sm">{Number(adjustAmount).toFixed(2)}</span>
              </div>
              )
            }
            {
              ...
              <span className="ml-2 text-sm">{Number(advanceAmount).toFixed(2)}</span>
            </div>
              )
            }
