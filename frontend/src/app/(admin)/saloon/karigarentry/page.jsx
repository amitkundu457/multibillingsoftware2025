...
export default function KarigarEntry() {
  ...
  const onSubmit = async (data) => {
    ...
    if (editingKarigar) {
      ...
      try {
        const response = await axios.post(" https://apibrize.brizindia.com/api/karigar-list", data, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleDelete = async (id) => {
    if (!token) return;
    try {
      const response = await axios.delete(` https://apibrize.brizindia.com/api/karigar-list/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
