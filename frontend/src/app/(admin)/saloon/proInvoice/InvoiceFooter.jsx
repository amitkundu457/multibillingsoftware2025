...
const InvoiceFooter = ({ data, taxes, companyName }) => {
  ...
  const totalTaxableValue = data?.details?.reduce(
    (sum, invoice) => sum + Number(invoice?.pro_total || 0),
    0
  );

  const totalCGST = data?.details?.reduce(
    (sum, invoice) =>
      sum + ((invoice?.pro_total || 0) * ((invoice?.tax_rate || 0) / 2)) / 100,
    0
  );
  ...
  const fetchTerms = async () => {
    ...
    try {
      const res = await axios.get(" https://apibrize.brizindia.com/api/terms-condition-invoice", {
        headers: { Authorization: `Bearer ${token}` },
      });

      if (res?.data?.length > 0) {
        setContent(res.data[0]?.content || "");
        setTermId(res.data[0]?.id || null);
      }
    } catch (error) {
      console.error("Fetch error:", error);
      setMessage("❌ Failed to load terms.");
