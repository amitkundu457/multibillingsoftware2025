...
export default function BookingModal({ customer_id }) {
  ...
  const fetchNumber = () => {
    axios
      .get("https://api.equi.co.in/api/packagesnext-numbers")
      .then((res) => {
  ...
  const fetchPackageByNumber = async () => {
    ...
    if (!packageNo) {
      toast.error("Please Enter a Package No");
    ...
    try {
      const response = await axios.get(
        `https://api.equi.co.in/api/packagesassn/${packageNo}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const onSubmit = (data) => {
    const apiUrl =
      mode === "new"
        ? `https://api.equi.co.in/api/packagesassign/${customer_id}`
    ...
    method(apiUrl, data, {
      ...
      .then((res) => {
        ...
        // );
        if(mode === "new"){
          toast.success("New package saved successfully!");
        }
        else{
      ...
      .catch((err) => {
        ...
        toast.error("Already has an Active Package");
