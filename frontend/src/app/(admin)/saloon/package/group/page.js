...
export default function Group() {
  ...
  const fetchSubtypes = async () => {
    const response = await axios.get(" https://apibrize.brizindia.com/api/membership-groups");
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (editingId) {
      await axios.put(` https://apibrize.brizindia.com/api/membership-groups/${editingId}`, { name });
    } else {
      await axios.post(" https://apibrize.brizindia.com/api/membership-groups", { name });
    }
    setName("");
    setEditingId(null);
    fetchSubtypes();
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this subtype?")) {
      await axios.delete(` https://apibrize.brizindia.com/api/membership-groups/${id}`);
      fetchSubtypes();
    }
  };
