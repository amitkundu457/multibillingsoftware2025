...
export default function Subtypes() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (editingId) {
      await axios.put(
        `  https://apibrize.brizindia.com/api/packagesubtypes/${editingId}`,
        { name }
      );
    } else {
      await axios.post("  https://apibrize.brizindia.com/api/packagesubtypes", {
        name,
      });
    }
    setName("");
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this subtype?")) {
      await axios.delete(
        `  https://apibrize.brizindia.com/api/packagesubtypes/${id}`
      );
      fetchSubtypes();
    }
  };
