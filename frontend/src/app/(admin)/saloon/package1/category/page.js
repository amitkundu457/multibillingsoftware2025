...
export default function CategoryPackage() {
  ...
  const fetchSubtypes = async () => {
    const response = await axios.get("  https://apibrize.brizindia.com/api/package-category");
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (editingId) {
      await axios.put(`  https://apibrize.brizindia.com/api/package-category/${editingId}`, { name });
    } else {
      await axios.post("  https://apibrize.brizindia.com/api/package-category", { name });
    }
    setName("");
    setEditingId(null);
    fetchSubtypes();
