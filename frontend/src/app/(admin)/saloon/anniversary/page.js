...
const CustomersCelebrate = () => {
  ...
  const fetchCustomers = async () => {
    try {
      ...
      const { data } = await axios.get("  https://apibrize.brizindia.com/api/customerTodayAnniversery", config);
      setCustomers(data);
    } catch (error) {
  ...
  const sendBulkMessages = async () => {
    ...
    try {
      ...
      await axios.post("  https://apibrize.brizindia.com/api/bulksendmessage", { customer_ids: selectedCustomers }, config);
  ...
  return (
    <div className="p-6">
      <h1 className="mb-4 text-2xl font-bold">Today Marriage Anniversary</h1>
      <button
