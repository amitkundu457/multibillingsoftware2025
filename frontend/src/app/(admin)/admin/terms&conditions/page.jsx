...
const TermsConditionPage = () => {
  ...
  const fetchTermsConditions = async () => {
    try {
      const response = await axios.get("  https://apibrize.brizindia.com/api/terms-condition");
  ...
  const handleSaveTerm = async () => {
    if (isEditMode && termsConditions.length==1 ) {
      ...
      try {
        await axios.put(`  https://apibrize.brizindia.com/api/terms-condition/${modalData.id}`, modalData);
      ...
      // Create new term
      try {
        await axios.post("  https://apibrize.brizindia.com/api/terms-condition", modalData);
        fetchTermsConditions();
        setIsModalOpen(false);
      } catch (error) {
  ...
  const handleDeleteTerm = async (id) => {
    try {
      await axios.delete(`  https://apibrize.brizindia.com/api/terms-condition/${id}`);
      fetchTermsConditions();
    } catch (error) {
