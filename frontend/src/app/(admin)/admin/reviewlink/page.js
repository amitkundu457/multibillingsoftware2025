...
//     useEffect(() => {
//         axios.get("  https://apibrize.brizindia.com/api/user-infos")
//             .then((response) => {
...
export default function ShopList() {
  ...
  useEffect(() => {
    axios
      .get(" https://apibrize.brizindia.com/api/user-infos")
      .then((response) => {
        console.log("API Response:", response.data);
  ...
  const handlePrint = () => {
    if (printRef.current) {
      ...
      printWindow.document.write(
        "<html><head><title>Print QR Codes</title></head><body>"
      );
      printWindow.document.write(printRef.current.innerHTML);
      printWindow.document.write("</body></html>");
      printWindow.document.close();
          ...
          shops.map((shop, index) => (
            <div key={index} className="mb-4">
              ...
              {showQRCodes && (
                <div className="mt-2">
                  <QRCode
                    // https://apibrize.brizindia.com/review/?slug=niraj-roy-urx2mw
                    value={` https://apibrize.brizindia.com/review/?slug=${shop.slug}`}
                    size={128}
                  />
                </div>
              )}
