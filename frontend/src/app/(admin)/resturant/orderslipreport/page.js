  ...
  const fetchOrders = async (startDate = "", endDate = "") => {
    ...
    try {
      const response = await axios.get(
        " https://api.equi.co.in/api/orderreport",
        {
          ...
          <tbody>
            {filteredOrders.map((order) => (
              <tr key={order.id}>
                ...
                <td className="border px-4 py-2">
                  {order.users?.customers?.phone || "N/A"}
                </td>
                <td className="border px-4 py-2">{order.gross_total || 0}</td>
                <td className="border px-4 py-2">
                  {order.payments?.price || 0}
                </td>
                <td className="border px-4 py-2">
                  {order.delivery_date
                ...
                <td className="border px-4 py-2">
                  <div className="flex gap-2">
                    <Link
                      href={`/admin/printinvoice/${order.id}`}
...
export default OrderSlipReport;
