...
const BulkSmsSender = () => {
  ...
  const fetchCustomerType=async ()=>{
     const token = getCookie("access_token");
       ...
       const response =  await axios.get('http://127.0.0.1:8000/api/customerstype',{headers}
...
const fetchCustomerSubType=async ()=>{
   const token = getCookie("access_token");
       const headers = token ? { Authorization: `Bearer ${token}` } : {};
       const response =  await axios.get('http://127.0.0.1:8000/api/customersubtypes',{headers}
 ...
 setCustomerSubTypeData(response.data);
  ...
  const sendBulkSms = async () => {
    ...
    try {
       const token = getCookie("access_token");
       const headers = token ? { Authorization: `Bearer ${token}` } : {};
       console.log(customerTypeId,customerSubTypeId);
       
      
      await axios.post('http://127.0.0.1:8000/api/bulk-sms', {
  ...
  const filteredSubTypes = customerTypeId ? customerSubTypeData.filter(
  ...
  return (
    <div className="p-4 max-w-md mx-auto space-y-4">
      ...
      <button
        className={`w-full p-3 text-white font-semibold ${sending ? 'bg-gray-400' : 'bg-blue-600'} rounded`}
        disabled={sending}
        onClick={sendBulkSms}
      >
