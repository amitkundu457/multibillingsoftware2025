...
export default function PurchaseReturn() {
    ...
    const fetchSalesReturn = async () => {
      try {
        const response = await axios.get(
          "https://apibrize.brizindia.com/api/jwel-sales-returns",
          {
  ...
  const fetchPayments = async () => {
    try {
      const response = await axios.get(
        "https://apibrize.brizindia.com/api/saloon-sales-returns",
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(
        "https://apibrize.brizindia.com/api/jwel-sale-returns",
        formData,
        {
          headers: {
              ...
              {currentPayments.length > 0 ? (
                currentPayments.map((payment, index) => (
                  <tr key={index} className="hover:bg-gray-100">
                    ...
                    <td className="p-2">
                      {payment?.sale_return_payments?.[0]?.payment_type}
                    </td>
                    <td className="p-2">
                      {payment?.sale_return_payments?.[0]?.amount}
                    </td>
