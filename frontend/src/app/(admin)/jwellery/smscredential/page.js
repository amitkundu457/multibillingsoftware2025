...
export default function SmsCredentialForm() {
  ...
  const fetchCredentials = async () => {
    try {
      const res = await axios.get(" https://apibrize.brizindia.com/api/sms-credentials");
  ...
  const handleSubmit = async (e) => {
    ...
    if (credentialId) {
      await axios.put(` https://apibrize.brizindia.com/api/sms-credentials/${credentialId}`, form);
    } else {
      const res = await axios.post(" https://apibrize.brizindia.com/api/sms-credentials", form);
  ...
  const handleDelete = async () => {
    if (credentialId && confirm("Are you sure to delete this SMS credential?")) {
      await axios.delete(` https://apibrize.brizindia.com/api/sms-credentials/${credentialId}`);
      setForm({
        business_name: "",
        sms_username: "",
        sms_password: "",
