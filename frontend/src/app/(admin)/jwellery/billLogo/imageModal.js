...
const LogoUploadModal = ({ open, onClose, masterId, currentLogo }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (masterId) {
        ...
        response = await axios.put(`  https://apibrize.brizindia.com/api/masterlogobill/${masterId}`, formData, config);
      } else {
        // Create a new record with the logo
        response = await axios.post("  https://apibrize.brizindia.com/api/masterlogobill", formData, config);
      }
  ...
  return (
    <Modal open={open} onClose={onClose} center>
      ...
      <div>
        {currentLogo && !logo && (
          <img src={`  https://apibrize.brizindia.com/${currentLogo}`} alt="Current Logo" style={{ maxWidth: "100px" }} />
        )}
