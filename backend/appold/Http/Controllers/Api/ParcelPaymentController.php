...
public function store(Request $request)
{
    ...
    // $order = ParcelBill::findOrFail($request->order_id);
    $order=\DB::table('parcel_orders')->where('id',$request->order_id)->first();
    $orderp=ParcelBill::where('parcel_order_id',$order->id)->first();
    $order = \DB::table('parcel_orders')->where('id', $request->order_id)->first();
...
// if (!$orderp) {
//     return response()->json(['error' => 'Bill not found for this order.'], 404);
// }

// $grandTotal = $orderp->grand_total;
